...
from . import serviceUtil, pageUtil, fileUtil, vulnUtil, pocUtil, pocModelUtil, ExpUtil, IpModelUtil, AliveModelUtil, \
    resultUtil

port_dict = {
    "0": '21, 22, 2049, 3389, 80, 443, 8080, 8081, 7001, 3306, 1433, 1521, 6379, 2375, 5432, 8443',
    "1": '80, 81, 8080, 8081, 3306, 1433, 5432',
    "2": '80-85, 8000-8100',
    "3": '21-23,80-90,135,137,161,389,443,445,873,1099,1433,1521,1900,2082,2083,2222,2375,2376,2601,2604,3128,3306,3311,3312,3389,4440,4848,5432,5560,5900-5902,6082,6379,7001-7010,7778,8009,8080-8090,8649,8888,9000,9200,10000,11211,27017,28017,50000,51111,50030,50060',
    "4": '0-65535',
}
    ...
    try:
        ...
        if mode == "service" or mode == "fofa":
            ...
            if mode == "fofa":
                ...
                ctx["fofa_size"] = conf2.get("setting", "FOFA_SIZE")
                ctx["fofa_test"] = conf2.get("setting", "FOFA_TEST").lower() == "true"
        elif mode == "vuln":
            ...
            result_list = resultUtil.get_results(task_id, isAll=True, page=page, each_num=each_num, mode=mode)
        ctx["task"] = task
...
def start_scan(request: HttpRequest):
    if request.method == "POST":
        ...
        if mode == 'service':
            if "start" in request.POST and request.POST["start"] == "true":  # 漏洞扫描是否开始
                isStart = True
            else:
                isStart = False
            ips = request.POST["ips"].strip()
            port_list = port_dict[request.POST["port"]].split(",") if request.POST["port"] != '-1' else request.POST["port2"].split(
                ",")
            all_port_list = []
            for i in port_list:
                i = i.replace(" ", "")
                if "-" in i:
                    for j in range(int(i.split("-")[0]), int(i.split("-")[-1])+1):
                        all_port_list.append(j)
                else:
                    all_port_list.append(int(i))
            ...
            try:
                ...
                page = 1
            if not serviceUtil.fofa_scan(query, False, description, page):
                return HttpResponse("fail")
        elif mode == "ip":
            ...
            if not AliveModelUtil.alive_scan(ips, type, gid):
                return HttpResponse("fail")
...
@_auth()
def alive_scan(request: HttpRequest):
    return scan(request, "alive", "1=1")

@_auth()
def alive_list(request: HttpRequest):
    return task_list(request, "alive")
