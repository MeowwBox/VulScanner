...
FOFA_TEST = conf.get("setting", "FOFA_TEST").lower() == "true"  # 是否检验url有效
...
type_dict = {
    "high": [22, 1099, 2049, 2375, 3389],
    "medium": [21, 1433, 1521, 3306, 5432, 6379]
}
        ...
        try:
            if self.url == "" and "http" not in self.webvpn:
                ...
                if "http" in self.webvpn:
                    ...
                    resp = requestUtil.get(url, cookies=self.cookies)
                    if "fail" in resp.text or "Not Found" in resp.text or "访问出错" in resp.text or "访问被拒绝" in resp.text:
                        return
                else:
                    global FOFA_TEST
                    url = self.url.strip("/")
...
def fofa_scan(query, isStart=False, description="", page=1):
    global FOFA_SIZE
    ...
    task = ScanTask(ip_range=query.replace(' && country="CN" && region != "HK"', '') + f" && page={page}",
                    task_count=len(results),
                    isStart=isStart, mode="fofa", description=description)
    task.save()
    tid = task.id
    scan_list = []
    count = 0
...
def change_fofa(fofa_size, fofz_test):
    global FOFA_SIZE, FOFA_TEST
    FOFA_SIZE = fofa_size
    FOFA_TEST = fofz_test
    conf.set("setting", "FOFA_SIZE", FOFA_SIZE)
    conf.set("setting", "FOFA_TEST", FOFA_TEST)
    fh = open((os.path.dirname(os.path.abspath("settings.py"))) + "\config.ini", "w")
    conf.write(fh)
    fh.close()
    print(FOFA_SIZE)
