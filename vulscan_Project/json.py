...
def get_async_result(request: HttpRequest):  # 伪异步，获取实时扫描结果
    ...
    if mode == "service" or mode == "fofa":
        ...
        for i in new_result:
            count += 1
            new_rows += alive_row.format(id=count, ip=i.ip, flag=i.flag, mode=i.mode)
    elif mode == "pwd":
        new_result = Pwd.objects.order_by("system").filter(system__icontains=request.GET["system"])
        for i in new_result:
            count += 1
            new_rows += pwd_row.format(id=count, system=i.system, username=i.username, password=i.password, p_id=i.id)
    elif mode == "task":
        each_num = 15  # 每页显示行数
...
def change_fofa(request: HttpRequest):
    fofa_size = request.GET["fofa_size"]
